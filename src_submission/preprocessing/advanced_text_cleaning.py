...
TEENCODE_NEUTRAL = {
    ...
    # === PHÂN BIỆT VÙNG MIỀN ===
    # ⚠️ CHỈ chuẩn hóa lỗi chính tả, KHÔNG chuẩn hóa biến âm (parky, backy, pake...)
    # Lý do: Giữ các biến âm để model học được intensity gradient
    "bắc kì": "bắc kỳ", "bac ky": "bắc kỳ",  # Lỗi chính tả → chuẩn
    "nam kì": "nam kỳ", "nam ky": "nam kỳ",  # Lỗi chính tả → chuẩn
    # NOTE: "parky", "backy", "pake" → GIỮ NGUYÊN (trong INTENSITY_SENSITIVE)
    
    "đ mời":"đéo mời",
    "namky": "nam kỳ", "coa": "có", "cmnr": "con mẹ nó rồi",
...
TEENCODE_INTENSITY_SENSITIVE = {
    ...
    "đm", "dm", "đmmm",
    "đcmm", "dcmm",
    "vkl", "vlon", "vleu", "vãi l", "vãi lòn", "vãi lìn",
    ...
    "parky", "backy", "paki", "pakky", "bake", "pake",
    "bắc kì", "bắc kỳ", "bac ky",
    "cali",
    "ba que", "3que", "3 /", "3 ke", "3q", "3 gậy", "3 sọc", "///",
    "fú pàk",
    
    # === SLANG MỚI (New slang - preserve) ===
    "khát nước",  # Slang mới
    "đờ u đu cờ ang cang huyền càng",  # Slang mới
...
class PersonNameDetector:
    ...
    def _compile_patterns(self):
        ...
        # CRITICAL: Title is case-insensitive, but Name MUST start with capital letter
        titles_regex = '|'.join(self.titles)
        # Make titles case-insensitive by adding both cases
        titles_case_insensitive = '|'.join([f'(?:{t}|{t.capitalize()}|{t.upper()})' for t in self.titles])
        self.pattern_title = re.compile(
            rf'\b({titles_case_insensitive})\s+([A-ZÀÁẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÈÉẺẼẸÊẾỀỂỄỆÌÍỈĨỊÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÙÚỦŨỤƯỨỪỬỮỰỲÝỶỸỴ][a-zàáảãạăắằẳẵặâấầẩẫậđèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữựỳýỷỹỵ]+)\b',
            re.UNICODE  # Removed re.IGNORECASE - name MUST be capitalized
        )
