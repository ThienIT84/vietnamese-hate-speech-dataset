"""
Convert PhoBERT segmented data to ViDeBERTa raw format - FIXED V2
...
Strategy: Use unique placeholders that don't contain underscores
"""
...
def protect_special_tokens(text):
    """
    Temporarily replace special tokens with UNIQUE placeholders
    that won't be affected by underscore removal
    """
    if pd.isna(text):
        return text, {}
    ...
    # Replace with UNIQUE placeholders (no underscores!)
    for i, token in enumerate(special_tokens):
        if token in text:
            # Use format that won't be affected by underscore removal
...
def remove_segmentation_safe(text):
    ...
    Steps:
    1. Protect special tokens (replace with unique placeholders)
...
def verify_special_tokens(df, text_col):
    ...
    for token in special_tokens:
        count = df[text_col].astype(str).str.contains(re.escape(token), regex=True).sum()
        
        if count > 0:
            print(f"\n✅ {token}: {count} samples")
            
            # Show examples
            samples = df[df[text_col].astype(str).str.contains(re.escape(token), regex=True)][text_col].head(2)
...
def main():
    print("="*80)
    print("🔄 CONVERTING SEGMENTED DATA TO RAW FORMAT - FIXED V2")
    print("="*80)
    ...
    # Convert
    print("\n✅ Converting with SAFE method (V2)...")
    ...
    for i in range(min(10, len(df))):
        ...
        if '<emo pos>' in after or '<emo neg>' in after:
            print(f"   ❌ WARNING: Special token broken!")
        elif '</s>' in after or '<s>' in after:
            print(f"   ❌ WARNING: BOS/EOS not removed!")
        elif 'XSPECIAL' in after:
            print(f"   ❌ WARNING: Placeholder not restored!")
        else:
            print(f"   ✅ OK")
