...
### **Mục đích**
Xử lý lại file đã có `input_text` (như `labeling_task_Thien.csv`), **rebuild lại từ raw_comment và raw_title** để giữ đúng emoji processing.

⚠️ **QUAN TRỌNG**: Mode này rebuild `input_text` từ `raw_comment` và `raw_title`, KHÔNG re-clean từ `input_text` cũ. Lý do: `input_text` đã mất emoji (đã chuyển thành tags hoặc removed).

### **Input Format**
```csv
id,input_text,raw_comment,raw_title,label
123,boy phố </s> nguuuu vcl,nguuuu vcl 😡,Boy phố,1
456,đẹp quá </s> yêu m,đẹp quá yêu m ❤️,Confession,0
```
...
### **Output Format**
```csv
id,input_text,input_text_original,raw_comment,raw_title,label
123,boy phố </s> ngu <very_intense> vãi lồn <emo_neg>,boy phố </s> nguuuu vcl,nguuuu vcl 😡,Boy phố,1
456,confession </s> đẹp quá yêu em <emo_pos>,đẹp quá </s> yêu m,đẹp quá yêu m ❤️,Confession,0
```

**Đặc điểm:**
- ✅ Giữ nguyên tất cả columns và metadata
- ✅ Backup original vào `input_text_original`
- ✅ **Rebuild từ raw_comment + raw_title** (giữ đúng emoji!)
- ✅ Full pipeline: emoji → tags, teencode, context "m", intensity markers
- ✅ Person masking từ raw_title (Tàng Keng → <person>)
